...
Appendix~\ref{app:data}. All errors are our own. We welcome collaboration on
extensions and related work; interested readers are invited to contact the
...
\texttt{ankleshrawat5@duck.com}. Corresponding author.}\\[2pt]
{\small ORCID:~\href{https://orcid.org/0009-0001-2399-8032}{0009-0001-2399-8032}}
\and Shreshtha Rawat\thanks{Independent researcher.}\\[2pt]
{\small ORCID:~\href{https://orcid.org/0009-0005-2546-2732}{0009-0005-2546-2732}}}

\date{This version: July 2026 (Working paper)}
...
options trading; investor protection; SEBI; difference-in-differences;
event study; market microstructure; India\\
...
$+2.2$ log points per month, $p=0.002$), and our preferred specification allows
...
of \citet{conleytaber2011}. Table~\ref{tab:pretrend} collects the pre-trend
and placebo diagnostics for the identifying gap in one place.

\begin{table}[H]
\centering
\caption{Pre-trend diagnostics for the identifying gap}
\label{tab:pretrend}
\small
\begin{threeparttable}
\begin{tabular}{lcc}
\toprule
 & Notional & Premium \\
\midrule
Pre-period differential trend $\hat\gamma$ (log pts/month) & $0.022$ & $-0.033$ \\
\quad standard error & $(0.007)$ & $(0.006)$ \\
\quad $p$-value & $[0.002]$ & $[<0.001]$ \\
\addlinespace
Placebo shift, false Nov 2023 adoption ($\hat\beta_{\mathrm{plac}}$) & $0.243$ & $-0.387$ \\
\quad standard error & $(0.140)$ & $(0.065)$ \\
\quad months (pre-period only) & $31$ & $19$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
\item Notes: $\hat\gamma$ is the slope of $\Delta_t=\ln(\text{index-opt.\
ADT})-\ln(\text{stock-opt.\ ADT})$ on a linear trend over the pre-period
(April 2022--October 2024 for notional; April 2023--October 2024 for
premium); Newey--West (6-lag) standard errors. The placebo assigns a false
adoption date of November 2023 and estimates equation~\eqref{eq:collapsed}
on pre-period data only (Table~\ref{tab:robust}, Panel A). The notional
placebo is small and insignificant at 5\% ($p=0.083$); the significant
\emph{premium} placebo is the mechanical signature of the strong
pre-existing premium-compression trend ($\hat\gamma=-0.033$), which the
trend-adjusted estimator absorbs by construction, its placebo analogue being
$\hat\gamma$ itself.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Table~\ref{tab:horizons} distills the headline magnitudes at the horizons
that matter for policy.

\begin{table}[H]
\centering
\caption{Headline effects at key horizons (trend-adjusted DiD vs.\ stock
options)}
\label{tab:horizons}
\small
\begin{threeparttable}
\begin{tabular}{lcc}
\toprule
 & Log-point gap & Percent \\
\midrule
\multicolumn{3}{l}{\emph{Panel A: Notional ADT (trend-adjusted)}}\\
\quad On impact ($h=0$) & $-0.573$ & $-43.6\%$ \\
\quad 3 months ($h=3$) & $-0.546$ & $-42.1\%$ \\
\quad 6 months ($h=6$) & $-0.519$ & $-40.5\%$ \\
\quad Mean, Nov 2024--May 2025 & $-0.557$ & $-42.7\%$ \\
\quad \emph{Memo:} no trend adjustment ($\gamma=0$) & $-0.469$ & $-37.4\%$ \\
\addlinespace
\multicolumn{3}{l}{\emph{Panel B: Premium ADT (trend-adjusted)}}\\
\quad On-impact level shift ($\beta$) & $-0.007$ & $-0.7\%$ \\
\quad Post-reform trend break ($\delta$, per month) & $+0.059$ & \\
\quad Year on year, Dec--May (reference) & & $-13.0\%$ \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
\item Notes: derived from Table~\ref{tab:main}, columns (5)--(6), and
Section~\ref{subsec:sens}. Panel A reports the trend-adjusted implied gap
$\beta+h\hat\delta$ ($\hat\beta=-0.573$, $\hat\delta=0.009$); the honest
short-run bracket $[-43\%,-37\%]$ is spanned by the trend-adjusted and the
no-trend ($\gamma=0$) window means. Panel B: the premium effect is the
reversal of the pre-reform premium-to-notional compression, captured by the
trend break $\delta$ ($+5.9$ log points per month, $p<0.001$) rather than a
level change; see Section~\ref{subsec:composition}. Percent $=100(e^{x}-1)$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Figure~\ref{fig:premnot} shows this compositional reversal directly: the log
premium-to-notional ratio falls steadily through the boom and turns upward at
the reforms.

\begin{figure}[H]
\centering
\IfFileExists{figures/fig9_premium_notional.pdf}
{\includegraphics[width=0.85\textwidth]{fig9_premium_notional.pdf}}
{\IfFileExists{fig9_premium_notional.pdf}
{\includegraphics[width=0.85\textwidth]{fig9_premium_notional.pdf}}
{\IfFileExists{../figures/fig9_premium_notional.pdf}
{\includegraphics[width=0.85\textwidth]{fig9_premium_notional.pdf}}
{\fbox{\parbox[c][4cm][c]{0.8\textwidth}{\centering [\,fig9\_premium\_notional.pdf:
run \texttt{code/07\_premium\_notional\_figure.py} to generate this figure\,]}}}}}
\caption{Composition of index-options activity: the log premium-to-notional
ratio $\ln(\text{premium ADT}/\text{notional ADT})$, market-wide (NSE+BSE),
April 2023--March 2026. Dashed line: linear trend fitted on the pre-reform
period (April 2023--October 2024) and extrapolated. Vertical lines: the three
reform waves (20 November 2024; January 2025; 1 April 2025). The multi-year
decline (progressively cheaper, shorter-dated, deeper out-of-the-money
contracts) reverses sharply after the first wave.}
\label{fig:premnot}
\end{figure}
...
evidence.
\emph{Ideas for India}, 29 April 2026. Accessed 16 July 2026.
...
so premium series start April 2023. Table~\ref{tab:bulletins} lists the
issues used and the panel months each contributes.

\begin{table}[H]
\centering
\small
\caption{SEBI Monthly Bulletin issues used and the panel months each
contributes}
\label{tab:bulletins}
\begin{threeparttable}
\begin{tabular}{lll}
\toprule
Bulletin issue & Panel months contributed & Options series \\
\midrule
April 2023 & Apr 2022 -- Mar 2023 (FY2022--23) & Notional only\tnote{a} \\
April 2024 & Apr 2023 -- Mar 2024 (FY2023--24) & Notional + premium \\
March 2025 & Apr 2024 -- Feb 2025 & Notional + premium \\
April 2025 & Mar 2025\tnote{b} & Notional + premium \\
April 2026 & Apr 2025 -- Mar 2026 (FY2025--26) & Notional + premium \\
\bottomrule
\end{tabular}
\begin{tablenotes}\footnotesize
\item Notes: annexure tables ``Trends in Equity Derivatives Segment at
NSE/BSE'' and ``Trends in Cash Segment of NSE/BSE''
(\url{https://www.sebi.gov.in}). Each issue reports monthly rows only for the
fiscal year then in progress; the notional panel spans 48 months (April
2022--March 2026) and the premium panel 36 months (April 2023--March 2026).
\item[a] The FY2022--23 bulletin format reports options turnover in notional
terms only; premium reporting begins with the FY2023--24 format.
\item[b] March 2025 cells extracted from the PDF annexure; stock-options
cells absent from that layout are recovered as fiscal-year totals minus the
sum of April 2024--February 2025.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
and figure code, and the source SEBI bulletin documents) is openly
available. The version-controlled project is maintained at
\url{https://github.com/DogInfantry/sebi-fo-curbs-2025}; a citable
snapshot of that repository is archived on Zenodo
(DOI: \href{https://doi.org/10.5281/zenodo.21419533}{10.5281/zenodo.21419533}),
and the curated dataset is deposited at the Harvard Dataverse
(DOI: \href{https://doi.org/10.7910/DVN/FZYZNN}{10.7910/DVN/FZYZNN}).
All inputs are public SEBI publications.
...
statistical code, and preliminary drafting
